...
{-|
Module : ExprDiff
Description : Has functions for evaluating and simplifying expressions, as well as computing partial and anit derivatives. 
...
class DiffExpr a where
  ...
  -- | Function that is used to calculate the partial derivative of an expression
  partDiff :: String -> Expr a -> Expr a

  -- | Function that is used to calculate the anti d of an expression
...
instance (Num a, Eq a, Fractional a, Show a, Floating a, Ord a) => DiffExpr a where
  ...
  {-A neat way to represent anti derivatives that I found on https://github.com/barskyn/CS1XA3/blob/master/Assign3/assign3/ExprDiff.hs-}
    antiDeriv (Const a) = Mult (Const a) (Var "x")
    antiDeriv (Power (Const a) (Const b)) = Mult (Power (Const a) (Const b)) (Var "x")
    antiDeriv (Power (Var x) (Const (-1))) = Ln (Var x)
    antiDeriv (Power (Var x) (Const n)) = Mult (Power (Var x) (Const (n+1))) (Power (Const (n+1)) (Const (-1)))
